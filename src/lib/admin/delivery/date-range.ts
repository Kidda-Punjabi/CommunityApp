...
import type { DeliveryRangeId } from "@/lib/admin/delivery/constants";

export type ResolvedDeliveryRange = {
  id: DeliveryRangeId;
  start: Date;
  end: Date;
  previousStart: Date;
...
export function resolveDeliveryRange(
...
): ResolvedDeliveryRange {
  const todayYmd = formatLondonYmd(now);

  let startYmd: string;
  let endYmd: string;
  let label: string;
  let resolvedId: DeliveryRangeId = id;
  ...
  if (id === "custom" && custom?.from && custom?.to && custom.from <= custom.to) {
    ...
    startYmd = addLondonDays(todayYmd, -89);
    endYmd = todayYmd;
    label = "Last 90 days";
    resolvedId = "90d";
  } else {
