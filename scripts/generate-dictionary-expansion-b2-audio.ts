/**
 * Generate audio for dictionary expansion batch 2 only.
 *
 * Targets flashcards tagged topic_tags ⊇ 'dictionary_expansion_2026_b2'
 * (expected: 173 rows on Vocabulary - Master List).
 *
 ...
 * - Does NOT touch other flashcards (including dictionary_expansion_2026 / batch 1)
 *
 * Usage:
 *   npx tsx scripts/generate-dictionary-expansion-b2-audio.ts
 *   npx tsx scripts/generate-dictionary-expansion-b2-audio.ts --limit=5
 *   npx tsx scripts/generate-dictionary-expansion-b2-audio.ts --dry-run
 *   npx tsx scripts/generate-dictionary-expansion-b2-audio.ts --verify-only
 *   npx tsx scripts/generate-dictionary-expansion-b2-audio.ts --pause-ms=1500
 *
 * Requires .env.local:
...
const TAG = "dictionary_expansion_2026_b2";
const PRIOR_TAG = "dictionary_expansion_2026";
...
async function loadB2Cards(supabase: SupabaseClient): Promise<FlashcardRow[]> {
...
async function countPriorBatchAssets(supabase: SupabaseClient): Promise<{
  cards: number;
  assets: number;
  ...
  const { data: cards, error } = await supabase
    ...
    .select("id")
    .eq("deck_id", DECK_ID)
    .contains("topic_tags", [PRIOR_TAG]);

  if (error) throw new Error(`Failed to load prior batch: ${error.message}`);
...
async function main() {
  ...
  const cards = await loadB2Cards(supabase);
  console.log(`Loaded ${cards.length} flashcards with tag ${TAG} (deck ${DECK_ID}).`);
  if (cards.length !== 173) {
    console.warn(`Expected 173 cards; got ${cards.length}. Continuing with actual set.`);
  }

  const prior = await countPriorBatchAssets(supabase);
  console.log(
    `Prior batch (${PRIOR_TAG}): ${prior.cards} cards, ${prior.assets} flashcard audio_assets`,
    prior.byStatus
  );
  ...
  if (verifyOnly) {
    ...
    const priorAfter = await countPriorBatchAssets(supabase);
    console.log(`Prior batch unchanged check: ${priorAfter.assets} assets`, priorAfter.byStatus);
    if (!result.ok || result.withAsset !== cards.length || result.approved > 0) {
      process.exit(1);
    }
    console.log("OK — all b2 cards have audio_assets, none approved.");
    return;
  }
  ...
  console.log(
    `Prior batch (${PRIOR_TAG}) after run: ${priorAfter.assets} assets (was ${prior.assets})`,
    priorAfter.byStatus
  );

  if (priorAfter.assets !== prior.assets) {
    console.error("ERROR: prior batch audio_assets count changed — investigate.");
    process.exit(1);
  }
