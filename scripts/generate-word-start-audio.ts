/**
 * Generate Punjabi TTS for Word Start words from Gurmukhi script.
 *
 * Do not run until Gurupma has approved the seeded word list.
 *
 * Usage:
 *   npx tsx scripts/generate-word-start-audio.ts --dry-run
 *   npx tsx scripts/generate-word-start-audio.ts
 *
 * Requires .env.local:
 *   NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, ELEVENLABS_API_KEY
 *
 * Storage convention (same bucket as vowel_game_words.audio_pa_url):
 *   quiz-question-audio/word-start/{id}-pa.mp3
 */
...
const STORAGE_PREFIX = "word-start";
...
async function main() {
  ...
  if (!rows?.length) throw new Error("No word_start_game_words rows found");
  ...
  for (const row of rows) {
    ...
    const storagePath = `${STORAGE_PREFIX}/${row.id}-pa.mp3`;
    console.log(
